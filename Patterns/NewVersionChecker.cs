...
namespace NewVersionChecker
{
    ...
    #endregion
#else
    #region ReleaseObject for JSON.NET
        public class ReleaseObject
    {
        public string Url { get; set; }
        public string Html_url { get; set; }
    ...
    public class NewVersionChecker
    {
        ...
        {
#if UsingDefaultJSON
            return HasNewVersionDefault();
#else
            return HasNewVersionNewton();
#endif
        }
        ...
        {
#if UsingDefaultJSON
            return await HasNewVersionDefaultAsync();
#else
            return await HasNewVersionNewtonAsync();
#endif
        }

#if UsingDefaultJSON
        # region Default JSON
        public Boolean HasNewVersionDefault()
        {
            Boolean flag = false;
        ...
        public async Task<Boolean> HasNewVersionDefaultAsync()
        {
            Boolean flag = false;
            var currentRelease = GetCurrentRelease();
        ...
        public Boolean HasNewVersionNewton()
        {
            Boolean flag = false;
            var currentRelease = GetCurrentRelease();
                using (HttpWebResponse response = _Request(APIUrl, "GET", null))
                {
                    if (response.StatusCode != HttpStatusCode.OK)
                        return flag;
                    var r = GetObjectListFromJsonNewton<ReleaseObject>(response);
                    foreach (var unit in r)
                    {
                        var release = unit.Tag_name.Replace("v", String.Empty);
                        if (CompareVersions(release, currentRelease) > 0)
        ...
        /// </summary>
        /// <returns>true, if new version exists</returns>
        public async Task<Boolean> HasNewVersionNewtonAsync()
        {
            Boolean flag = false;
            var currentRelease = GetCurrentRelease();
                using (HttpWebResponse response = await _RequestAsync(APIUrl, "GET", null))
        ...
        /// <returns></returns>
        public static IList<T> GetObjectListFromJsonNewton<T>(HttpWebResponse response)
        {
            if (response.StatusCode != HttpStatusCode.OK)
                throw new WebException(response.StatusDescription);
            using (Stream oStream = response.GetResponseStream())
